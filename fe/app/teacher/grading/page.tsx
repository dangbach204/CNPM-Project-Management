...
export default function GradingPage() {
  ...
  const reviewedSubmissions = mySubmissions.filter(
    (s) => s.status === "reviewed"
  );
  const gradedSubmissions = mySubmissions.filter((s) =>
    mockGrades.some((g) => g.submissionId === s.id)
  );
  ...
  const averageScore =
    gradedSubmissions.length > 0
      ? Math.round(
          (mockGrades
            .filter((g) =>
              gradedSubmissions.some((s) => s.id === g.submissionId)
            )
            ...
            <div className="p-8 space-y-8">
              <div>
                ...
                <p className="text-muted-foreground mt-2">
                  Quản lý và chấm điểm bài nộp của sinh viên
                </p>
              </div>
