...
export default function ProjectDetailPage() {
  ...
  const isEnrolled = user
    ? project.enrolledStudents.includes(user.id.toString())
    : false;
  const studentSubmissions = mockSubmissions.filter(
    (s) => s.projectId === projectId && s.studentId === user?.id.toString()
  );
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                        ...
                        Sinh viên tham gia ({project.enrolledStudents.length}/
                        {project.maxStudents})
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
